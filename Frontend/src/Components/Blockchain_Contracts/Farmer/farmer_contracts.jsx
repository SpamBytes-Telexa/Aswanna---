...
export default function MyContracts() {
  ...
  const handleViewDispute = (contractId) => {
    alert(`${t("viewDisputeFor")}: ${contractId}`);
  };
  ...
  const statusBadge = (status) => {
    ...
    switch (status) {
      case "accepted":
        return <span className={`${base} bg-blue-100 text-blue-700`}>{t("accepted")}</span>;
      case "disputed":
        return <span className={`${base} bg-red-100 text-red-700`}>{t("disputed")}</span>;
      case "completed":
        return <span className={`${base} bg-green-100 text-green-700`}>{t("completed")}</span>;
      case "in_delivery":
        return <span className={`${base} bg-yellow-100 text-yellow-800`}>{t("inDelivery")}</span>;
      case "awaiting":
      default:
        return <span className={`${base} bg-gray-200 text-gray-700`}>{t("awaiting")}</span>;
    }
  };
            ...
            <table className="min-w-full divide-y divide-green-200 text-sm">
              <thead className="bg-green-100 text-green-900 text-sm uppercase tracking-wider">
                <tr>
                  ...
                  <th className="p-3 text-left">{t("prices")}</th>
                  <th className="p-3 text-left">{t("status")}</th>
                  <th className="p-3 text-left">{t("delivery")}</th>
                  <th className="p-3 text-left">{t("location")}</th>
                  <th className="p-3 text-left">{t("quantity")}</th>
                  <th className="p-3 text-left">{t("deadline")}</th>
              ...
              <tbody className="divide-y divide-green-100 bg-white">
                {contracts.map((c) => (
                  ...
                  >
                    <td className="p-3">{t(c.product)}</td>
                    <td className="p-3">Rs. {c.price}</td>
                    <td className="p-3">{statusBadge(c.status)}</td>
                    <td className="p-3">{t(c.delivery_method)}</td>
                    <td className="p-3">{t(c.location)}</td>
                    ...
                    <td className="p-3 space-y-1">
                      ...
                      {(c.status === "awaiting" || c.status === "in_delivery") && (
                        <span className="text-gray-500">⏳ {t("noAction")}</span>
                      )}
                    </td>
                  </tr>
                ))}
